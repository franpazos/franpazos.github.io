class Game {
  constructor() {
    ...
    this.player = new Player(this.gameScreen, "./img/popino.png");
    ...
    this.gameIntervalId;
    this.gameLoopFrequency = Math.round(1000 / 60); // 60 frames per second
  ...
  checkBottomReach() {
    const invadersReachedBottom = this.invaders.some((invader) =>
      invader.reachedBottom()
    );
  ...
  createBullet() {
    const bullet = new Bullet(
      this.gameScreen,
      this.player.position.left + this.player.dimensions.width / 2,
  ...
  checkBombCollision() {
    for (let i = 0; i < this.bombs.length; i++) {
      ...
      if (this.player.didCollide(bomb)) {
        ...
        bomb.position.top >
        this.gameScreen.clientHeight - bomb.dimensions.height
      ) {
        bomb.element.remove();
        this.bombs.splice(i, 1);
        i--;
